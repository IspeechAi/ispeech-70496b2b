  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (error) {
        if (error.message.includes('email') && error.message.includes('limit')) {
          toast({
            ...
            description: "Email verification is temporarily limited. Please try signing in if you already have an account, or try again later.",
            variant: "destructive",
          });
        } else {
          throw error;
      ...
      toast({
        ...
        description: error.message || "Please try again later or use Google sign-in.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/tts`
        }
      });
